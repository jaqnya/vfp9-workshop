...
/* -------------------------------------------------------------------------- *
 * FN para obtener un código nuevo para la tabla 'marca'.                     *
 * -------------------------------------------------------------------------- */
DELIMITER $$

DROP FUNCTION IF EXISTS fn_marca_codigo_nuevo $$

CREATE FUNCTION fn_marca_codigo_nuevo()
...
/* -------------------------------------------------------------------------- *
 * FN para averiguar si existe un código en la tabla 'marca'.                 *
 * -------------------------------------------------------------------------- */
DELIMITER $$

DROP FUNCTION IF EXISTS fn_marca_codigo_existe $$

CREATE FUNCTION fn_marca_codigo_existe(p_codigo SMALLINT UNSIGNED)
...
/* -------------------------------------------------------------------------- *
 * FN para averiguar si existe un nombre en la tabla 'marca'.                 *
 * -------------------------------------------------------------------------- */
DELIMITER $$

DROP FUNCTION IF EXISTS fn_marca_nombre_existe $$

CREATE FUNCTION fn_marca_nombre_existe(p_nombre VARCHAR(50))
    RETURNS TINYINT UNSIGNED
BEGIN
...
DROP PROCEDURE IF EXISTS pa_marca_modificar $$

CREATE PROCEDURE pa_marca_modificar(IN p_codigo SMALLINT UNSIGNED,
                                    IN p_nombre VARCHAR(50),
                                    IN p_vigente TINYINT)
BEGIN
    ...
    IF v_nombre <> p_nombre OR v_vigente <> p_vigente THEN
        UPDATE
            marca
        SET
            nombre = p_nombre,
